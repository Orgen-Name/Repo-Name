...
function deleteProvider(obj){
	$.ajax({
		type:"GET",
		url:path+"/user/delprovider.html",
		data:{proid:obj.attr("id")},
		dataType:"json",
...
$(function(){
	$(".viewProvider").on("click",function(){
		...
		var obj = $(this);
		window.location.href=path+"/user/providerview/"+ obj.attr("id");
	});
	
	$(".modifyProvider").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/user/providermodify/"+ obj.attr("id");
	});
